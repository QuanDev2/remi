...
// Milestones arrive from the criteria freeze; the planner assigns steps to them rather than
// inventing them. The grouping is Quan's priority order, settled in the interview, and it
// survives the plan being thrown away and rewritten.
//
// `order_concerns` is how the planner disagrees without regrouping: if a milestone cannot be
// built before the one after it, that is a fact about the code and it belongs in front of
// Quan at the gate, not quietly resolved by reordering what he asked to see first.
...
The milestones Quan settled in his interview, in the order he wants to see them working,
as JSON. Each names the criteria it delivers:
...
Produce an ordered list of steps, each assigned to one of the milestones above by its
id. Each step names the files it touches, the criterion ids it serves, and why it
exists. A step serving no criterion is scope creep, so leave it out. Add a risks list:
...
The milestones are Quan's, not yours to regroup. He decided what he wants working first,
and phase 2 builds one milestone, stops, shows him the result and waits. Your part is
deciding what work each one takes.

Two consequences for how you sequence. Preparation with no visible result — a migration,
an extraction, a module nothing calls yet — belongs inside the milestone that first needs
it, so that every milestone lands as something he can look at. And when a milestone
genuinely cannot be built before the one after it, because the code makes that order
impossible, put that in order_concerns and plan it in his order anyway. He reads
order_concerns at the gate and decides; a reordering you make quietly is a decision taken
away from him.

Expected answers, both correct:
- The criteria can be satisfied as they stand: status "ok", reason "", steps in execution
  order each naming its milestone, risks listed, order_concerns holding anything about the
  requested order that worries you.
- The criteria need a decision only Quan can make: status "blocked", reason naming
  that exact decision, steps holding whatever sequencing is already settled, risks and
  order_concerns listed. Naming the decision is the useful answer.
...
The milestones Quan settled, which are his and not the planner's to change, as JSON:
${JSON.stringify(goal.milestones, null, 2)}

The plan under review, as JSON:
${JSON.stringify(
  { steps: plan.steps, risks: plan.risks, order_concerns: plan.order_concerns },
  null,
  2,
)}
${FILE_ACCESS}
...
- Whether each milestone, given the steps assigned to it, actually delivers the criteria it
  claims. Phase 2 stops after each one and shows Quan the result, so a milestone whose steps
  only add a module nobody calls yet hands him rubble and wastes the stop. Preparation with
  no visible result belongs inside the milestone that needs it.
- Whether every criterion has steps under the milestone that claims it, and whether the
  planner's order_concerns are real. An order concern that is actually a preference for a
  tidier build order is worth saying so: Quan chose this order.
...
const goal = {
  ...
  // Quan's grouping, settled in the interview. A freeze written before milestones existed
  // has none, and the whole plan is then one stage — which keeps an older reference
  // runnable instead of stranded.
  milestones: goalRow.details.milestones || [],
...
if (goal.milestones.length === 0) {
  // One implicit milestone covering everything, so the planner has something to assign to
  // and phase 2 has something to select. Named so a reader can tell it was inferred.
  goal.milestones = [
    {
      id: "M1",
      name: "Whole plan",
      demonstrates: "Every criterion, delivered in one stage.",
      criteria: goal.criteria.map(function (c) {
        return c.id;
      }),
    },
  ];
  log("The freeze carries no milestones; treating the plan as one stage.");
}
...
const planRow = await writeEntry({
  ...
  content:
    plan.status === "ok"
      ? plan.steps.length +
        ...
        plan.risks.length +
        " risks" +
        (plan.order_concerns.length > 0
          ? ", " + plan.order_concerns.length + " order concern(s)"
          : "") +
        ". " +
        goal.milestones
          .map(function (m) {
  ...
  details: {
    ...
    steps: plan.steps,
    risks: plan.risks,
    order_concerns: plan.order_concerns,
    status: plan.status,
  },
});
...
// Every step must land in a milestone Quan approved, and every milestone must receive
// steps. Both failures are silent: an unassigned step is work phase 2 will never run, and
// an empty milestone is a stop with nothing to show.
const knownMilestones = {};
for (const m of goal.milestones) knownMilestones[m.id] = 0;
const strayAssignments = [];
for (const s of plan.steps) {
  if (knownMilestones[s.milestone] === undefined) {
    strayAssignments.push(s.id + " -> " + (s.milestone || "(none)"));
  } else {
    knownMilestones[s.milestone] += 1;
  }
}
const starvedMilestones = Object.keys(knownMilestones).filter(function (id) {
  return knownMilestones[id] === 0;
});

if (strayAssignments.length > 0 || starvedMilestones.length > 0) {
  await writeEntry({
    agent: "orchestrator",
    type: "finding",
    reference: reference,
    ...
    needs_human: true,
    content:
      "The plan does not map onto Quan's milestones." +
      (strayAssignments.length > 0
        ? " Steps assigned to unknown milestones: " + strayAssignments.join(", ") + "."
        : "") +
      (starvedMilestones.length > 0
        ? " Milestones with no steps: " + starvedMilestones.join(", ") + "."
        : "") +
      " Phase 2 builds one milestone per run, so an unassigned step never runs and an empty" +
      " milestone is a stop with nothing to show.",
    details: {
      stray_assignments: strayAssignments,
      starved_milestones: starvedMilestones,
      milestones: goal.milestones,
    },
  });
}
...
log(
  plan.steps.length +
    ...
    goal.milestones.length +
    " milestone(s) from the criteria freeze.",
);
